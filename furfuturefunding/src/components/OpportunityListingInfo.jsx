...
function OpportunityListingInfo({ opportunity }) {
  ...
  return (
    <div className="opportunity-container">
      ...
      <p className="opportunity-info">{`Scholarship type: ${opportunity.type.map((item) => item.description) || ""
        }`}</p>
      <p className="opportunity-info">{`Field of study: ${opportunity.discipline.map((item) => item.description) || ""
        }`}</p>
